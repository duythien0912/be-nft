        ...
        if (allNft[key].type === "playlist")
          return (
            <Row key={key} xs={12} className="m-0 mb-5 align-items-center">
              <Col xs={2} className="">
                <Image className="fadein" src={allNft[key].baseTokenURI} rounded fluid />
              </Col>
              <Col xs={8} className="text-left">
                <p className="mt-2 mb-0">Playlist</p>
                <p className="display-4 font-weight-bold mb-0">{allNft[key].name}</p>
                <p className="display-6 mb-1">
                  {allNft[key].couponTokenSymbol}
                </p>
                
                <p className="caption">{Math.floor(Math.random() * 99) + 1} Songs, {Math.floor(Math.random() * 9) + 1} hr {Math.floor(Math.random() * 50) + 1} min</p>
              </Col>
